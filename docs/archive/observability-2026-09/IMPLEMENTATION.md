...
依据：[实施方案](../../design/OBSERVABILITY.md)、[核心路径和任务](../../observability/ERROR-PATHS.md)。用户已授权启动开发，按顺序推进代码、测试和集成；云端 DSN/真实事件、设备与发布证据分别记录，不能由本地测试替代。
...
- D：新增 [queries](../../observability/queries.md)、[RELEASE](../../observability/RELEASE.md)、[PRODUCT-REVIEW](PRODUCT-REVIEW.md)，以及单次独立 HTTP 健康探测、真实回执 P95 汇总和 iOS archive UUID/可选符号上传脚本。脚本尚未执行；未创建长期监控、通知或公开发版。
