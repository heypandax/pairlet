...
| 新域路由 | SHA-512 核实后的 Caddy v2.10.2 成功解析候选；没有启动 Caddy 或部署。继承原配置的 header 提示及格式 warning 已记录，显式源 IP 覆盖未移除。 |
...
新域名的控制权/TLS、中国网络、Turnstile、支持链及新旧 relay 组合，当前 ASC 后台状态，
