...
- 移动端顶部「公开 · 无需登录 · 无账号历史」状态胶囊按可用宽度完整显示，窄屏自然换行，不使用省略号截断；运行会话最多保留 30 天，不再使用与真实存储不符的「不保存历史」。
- 首次匿名提问会在对话工作区内完成一次 Turnstile 安全验证，不跳转、不登录；验证组件位于输入框上方，不改变输入框聚焦尺寸。短时通行证只留在页面内存，服务端另有持久化访客/全局额度与每日 Token 预算熔断。
...
设计轮只重新设计入口流程、交互 UI、帮助内容层级、响应式布局与前端回退状态。2026-07-26 的实现修正额外改动了公开支持 API 的安全上下文白名单、元数据提示封装、内部旁白过滤与匿名防滥用边界；公开 Agent 已移除共享候选队列读写，已核验手册仍作为正式内容源。
