...
- App 只传递 `mode=chat` 与 `source=app` 两个非敏感参数，不上传页面、会话、文件或用户问题上下文。
...
本轮只重新设计入口流程、交互 UI、帮助内容层级、响应式布局与前端回退状态。现有智能客服后端、已核验知识库及其治理机制沿用既有实现，不重复设计，也未做任何代码改动。
