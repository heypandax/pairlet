...
- 收 `Transcript(ok=true)`：置 `pendingTranscript` state；ChatScreen 用 `LaunchedEffect` 消费并 **append 到 input**（input 是 ChatScreen 本地 `remember`，桥接走这条单向通道，不把输入框状态上提）。
...
| **P1（可选）** | iOS SFSpeechRecognizer 实时听写，可用即切、不可用回落 daemon | 实时 partial 上屏 |
