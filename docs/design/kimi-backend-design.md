...
| V3 | **PASS（ACP 事件序）** | 一轮内 `session/update` 序：`agent_thought_chunk`×N + `agent_message_chunk`×N + `tool_call`→`tool_call_update`×N（in_progress 流式）→ settled；轮末 prompt 响应 `{stopReason}`。另有 `session_info_update`（标题）、`usage_update{used,size}`（**占用线数据源，P2 接入**）、`available_commands_update`。无 `user_message_chunk`（消费回执缺席）。**后续修复（2026-08-08，评审发现）**：回执缺席导致 Conversation 的 prompt ledger 永不结算——任何 relaunch（切模型/进程重生/lock heal）会把全部历史 prompt 重投重跑，且 `maybeEndTaskOnSettle` 因 `hasUnconsumedPrompts()` 恒真使 task grants 不在轮边界消亡。已改 `KimiBackend`：`promptIds` 记 id→text，prompt settle（含 error settle，防重投失败循环）时**合成 `UserReplay(text)`** 作为消费回执，ledger 契约恢复成立。 |
| V5 | **实测改判** | 轮中再次 `session/prompt` 返回 `-32600 "another turn is already in progress"（turn.agent_busy）`——**ACP 无轮中注入**。已在 `KimiBackend` 内做 in-flight 门闩＋FIFO 排队（prompt 响应落地后按序 flush），手机端不再出现 turn failed。**后续修复（2026-08-08，评审发现）**：门闩登记原在锁外（`writePrompt` 末尾才进 `promptIds`），router 与 pump 并发下存在「看似 idle → 双发 → -32600 复现」窗口；已改为 id 预分配＋登记在 `bootstrap` 锁内原子完成（`reservePrompt`）。 |
