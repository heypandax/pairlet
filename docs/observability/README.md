...
从用户反馈推进到证据、日志补齐和测试包交付，使用[反馈排障流程](FEEDBACK-DIAGNOSTICS.md)。其中定义了日志验收以及 TestFlight 上传、外测审核和用户可安装的不同完成标准。
