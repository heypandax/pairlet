...
初始基线：`6ca601737b1c82381c66ef5075d534c146844390`；最初检查时 origin/main 为 `c6bf15ab`，GitHub 稳定版为 v1.9.8。早期品牌候选沿用 1.9.8 以便独立审阅差异；2026-09-10 已按用户决定将正式 Pairlet 候选版本设为 **2.0.0**。这不是发布或覆盖升级验收回执；正式升级试用前仍须核实各渠道构建号，不能用同版本覆盖证明升级通过。
