 ...
 *  - `ALLOW`  → safe enough for ordinary and legacy-confined bridge turns to run with zero owner
 ...
 *  - `ASK`    → routes to the owner's phone by default. An explicitly authorized #233 full-auto turn may
 ...
 * at the regex level. In ordinary and legacy-confined bridge turns, a bypassed DENY falls through to ASK;
 * this tiny ALLOW list + ASK-by-default is the actual boundary. Issue #233's separately and explicitly
 * confirmed FULL_AUTO grant may consume an ASK without another card, so that mode must be presented as
 * broad owner-delegated shell authority, never as a sandbox or a complete destructive-command barrier.
 */
