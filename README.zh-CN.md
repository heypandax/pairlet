...
**Windows（x86_64）** —— 从 [Releases](https://github.com/heypandax/cc-pocket/releases/latest) 下载 `cc-pocket-daemon-<version>-windows-x86_64.zip`。守护进程需要驱动 [Claude Code CLI](https://github.com/anthropics/claude-code)，请先装好它。然后**在 zip 所在的目录里**打开 PowerShell：
...
# 启动守护进程, 它会连上托管的 relay —— 保持这个窗口开着别关
& $ccp run
```

再开**第二个** PowerShell 窗口配对手机（第一个窗口的守护进程要一直运行）：

```powershell
& "$env:LOCALAPPDATA\Programs\cc-pocket-daemon\cc-pocket-daemon.exe" pair   # 打印一个二维码 + 6 位配对码
```

这个 zip 是自包含的（内置 JRE —— 无需系统 Java）；用 `-Force` 覆盖解压到原文件夹即可升级。想让它在后台常驻、不用一直开着窗口？`& $ccp service-install` 会打印 `sc.exe` 命令，但这个 launcher 不是原生 Windows 服务，需要用 [WinSW](https://github.com/winsw/winsw) 之类的工具包装（以管理员身份运行）。其他架构（Linux arm64）：请从源码构建 —— 见 [快速开始](#快速开始)。
