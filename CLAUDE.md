...
- **升级 claude CLI 后**跑 `python3 scripts/probe-claude-wire.py`——回归 daemon 依赖的三条 stream-json 行为（中途消息排队/注入、AskUserQuestion answers 形状），漂移会让排队与提问卡静默变坏；**升级 codex CLI 后**同理跑 `python3 scripts/probe-codex-wire.py`——回归 experimental 的 app-server 四条行为（握手/陈旧 steer 被带 id 的 error 拒绝/`thread/fork` 存在/未知方法错误形状），漂移会让提示词被吞与接管防双写静默失效；**升级 dsh CLI 后**跑 `python3 scripts/probe-dsh-acp.py`——回归 ACP profile 的九条行为（握手能力／`session/new`＋`resume` 的 configOptions 形状／`set_config_option` 的 `configId` 与不透明 model value／一条 prompt 在飞时的 `-32602`／`request_permission` 只带 toolCallId／`usage_update`），漂移会让模型切换与审批卡静默失灵。**dsh 必须 ≥ 0.1.2-rc.1**（旧版没有 acp profile，握手永不回应）。
