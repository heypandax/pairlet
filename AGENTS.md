...
遇到“用户反馈未解决”“核对/补充排障日志”“给用户发 TF 排查”，先按[用户反馈排障与测试版本交付](docs/observability/FEEDBACK-DIAGNOSTICS.md)推进；本地 `feedback-diagnostics` skill 是快捷入口，共享流程以仓库文档为准。面向外部用户的 TF 包应包含测试组配置和 Beta 审核提交，不能停在构建 `VALID`；明确仅构建或内部测试时遵循该范围。审核待处理、已可安装与故障已修复分别报告，不自动扩大成正式上架、其他平台发版或生产服务部署。
